...
void Network::Init(data_Network) 
{
	actFunc.set();
	srand(time(NULL));
	L = data.L;//запоминаем сколько слоаев в нейронке
	size = new int[L];
	for (int i = 0; i < L; i++) 
	{
		size[i] = data.size[i];
...
double Network::ForwardFeed() {
	...
	return pred;///речь о матрице 28 на 28 где наиб  те 1 выводим
}

int Network::SearchMaxIndex(double* value) {///принимет вектор значений
	double max = value[0];
	int prediction = 0;
	double tmp;
	for (int j = 1; j < size[L - 1]; j++) {
		...
		if (tmp > max) {
			prediction = j;
			max = tmp;
		}//ищем макс элемент возв его индекс
	}
	return prediction;
}

void Network::PrintValues(int L) {///на экран значение нейрона на этом слое
